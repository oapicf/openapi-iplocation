...
public interface DefaultApi {
    ...
    default ResponseEntity<Get200Response> rootGet(
        @NotNull @Parameter(name = "ip", description = "An IPv4 or IPv6 address that you would like to lookup.", required = true, in = ParameterIn.QUERY) @Valid @RequestParam(value = "ip", required = true) Object ip,
        @Parameter(name = "format", description = "Output format, the following formats are supported: plain xml json jsonp php csv serialized", in = ParameterIn.QUERY) @Valid @RequestParam(value = "format", required = false) Object format,
        @Parameter(name = "delimiter", description = "Delimiter between proxies. Can be used only with format plain. The following types are supported: 1 for \"\\n\", 2 for \"<br>\".", in = ParameterIn.QUERY) @Valid @RequestParam(value = "delimiter", required = false) Object delimiter
    ) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"response_code\" : \"\", \"ip_version\" : \"\", \"response_message\" : \"\", \"ip\" : \"\", \"isp\" : \"\", \"country_name\" : \"\", \"country_code2\" : \"\", \"ip_number\" : \"\" }";
