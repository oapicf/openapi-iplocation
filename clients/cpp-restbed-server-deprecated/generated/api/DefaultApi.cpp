...
void DefaultApiResource::handler_GET_internal(const std::shared_ptr<restbed::Session> session)
{
    ...
    const std::string ip = getQueryParam_ip(request);
    const std::string format = getQueryParam_format(request);
    const std::string delimiter = getQueryParam_delimiter(request);


    int status_code = 500;
    std::shared_ptr<__get_200_response> resultObject = std::make_shared<__get_200_response>();
    std::string result = "";

    try {
        std::tie(status_code, resultObject) =
             handler_GET(ip, format, delimiter);
    }
    ...
    if (status_code == 200) {
        ...
        returnResponse(session, 200, result.empty() ? "Successfully processed the request." : result, contentType);
        return;
    }
    if (status_code == 400) {
        result = resultObject->toJsonString();
        ...
        const constexpr auto contentType = "application/json";
        returnResponse(session, 400, result.empty() ? "Failed to complete the request." : result, contentType);
        return;
    }
    if (status_code == 404) {
        result = resultObject->toJsonString();
        ...
        const constexpr auto contentType = "application/json";
        returnResponse(session, 404, result.empty() ? "Command not found." : result, contentType);
        return;
    }
...
std::pair<int, std::shared_ptr<__get_200_response>> DefaultApiResource::handler_GET(
        std::string const & ip, std::string const & format, std::string const & delimiter)
{
