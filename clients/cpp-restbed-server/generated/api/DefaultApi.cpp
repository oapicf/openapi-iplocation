...
void Resource::handler_GET_internal(const std::shared_ptr<restbed::Session> session)
{
    ...
    std::string ip = request->get_query_parameter("ip", "");
    std::string format = request->get_query_parameter("format", "");
    std::string delimiter = request->get_query_parameter("delimiter", "");
    
    int status_code = 500;
    __get_200_response resultObject = __get_200_response{};
    std::string result = "";
    ...
    if (status_code == 400) {
        ...
        result = "Failed to complete the request.";
...
std::pair<int, __get_200_response> Resource::handler_GET(
        std::string & ip, std::string & format, std::string & delimiter)
{
    return handler_GET_func(ip, format, delimiter);
}
