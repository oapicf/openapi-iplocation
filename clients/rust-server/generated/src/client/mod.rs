        ...
        context: &C) -> Result<RootGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::Get200Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(RootGetResponse::SuccessfullyProcessedTheRequest
                    (body)
                )
            }
            400 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::Get400Response>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(RootGetResponse::FailedToCompleteTheRequest
                    (body)
                )
            }
            404 => {
                ...
                Ok(RootGetResponse::CommandNotFound
                    (body)
                )
            }
            code => {
