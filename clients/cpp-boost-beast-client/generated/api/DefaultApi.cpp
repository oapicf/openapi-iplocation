...
std::shared_ptr<__get_200_response>
DefaultApi::rootGet(
    const std::string& ip, const boost::optional<std::string>& format, const boost::optional<std::string>& delimiter) {
    ...
    const char* queryParameterSeparator = "?";
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "ip",
            serializeQueryParameterValue(ip));
    if (format) {
        appendQueryParameter(
            queryParameterStream,
            queryParameterSeparator,
    ...
    if (delimiter) {
        appendQueryParameter(
            ...
            queryParameterSeparator,
            "delimiter",
            serializeQueryParameterValue(*delimiter));
    }
    ...
    std::shared_ptr<__get_200_response> deserializedResponse = std::make_shared<__get_200_response>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<__get_200_response>>::deserialize(
    ...
    if (statusCode == boost::beast::http::status(400)) {
        throw DefaultApiException(statusCode, "Failed to complete the request.");
    }
    if (statusCode == boost::beast::http::status(404)) {
        throw DefaultApiException(statusCode, "Command not found.");
    }
